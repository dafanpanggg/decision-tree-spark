...
object DecisionTreeApplication {
  ...
  private def readInputArgs(args: Array[String]): Configuration = {
    ...
    try {
      args.length match {
        case 1 => Configuration.init(args(0))
        case 2 => Configuration.init(args(0), args(1))
        case _ => Configuration.init(args(0), args(1), args(2).toInt)
      }
    } catch {
      case _: Exception =>
        throw new IllegalArgumentException(
